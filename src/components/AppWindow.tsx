...
const Window = (props: WindowProps) => {
  ...
  const isMax = isMobileView || props.max;

  const round = isMax ? "rounded-none" : "rounded-lg";
  const minimized = props.min
    ? "opacity-0 invisible transition-opacity duration-300"
    : "";
  const border = isMax ? "" : "border border-gray-500/30";
  const width = isMax ? winWidth : state.width;
  const height = isMax ? winHeight : state.height;
  ...
  return (
    <Rnd
      ...
      position={{
        x: isMax
          ? (isMobileView ? 0 : winWidth) // on mobile, no boundary offset needed
          : Math.min(
        ...
        y: isMax
          ? (isMobileView ? 0 : -minMarginY) // on mobile, no boundary offset
          : Math.min(
              // "- minMarginY" because of the boundary for windows
