...
import anukul from "~/configs/anukul";
import type { AnukulMdData } from "~/types";
...
interface SidebarProps {
  cur: number;
  setMidBar: (items: AnukulMdData[], index: number) => void;
}

interface AnukulState extends ContentProps {
  curSidebar: number;
  curMidbar: number;
  midbarList: AnukulMdData[];
}
...
const Sidebar = ({ cur, setMidBar }: SidebarProps) => {
  return (
    <div text-white>
      ...
      <ul>
        {anukul.map((item, index) => (
          <li
            key={`anukul-sidebar-${item.id}`}
...
const Middlebar = ({ items, cur, setContent }: MiddlebarProps) => {
  return (
    <ul>
      {items.map((item: AnukulMdData, index: number) => (
        <li
          key={`anukul-midbar-${item.id}`}
...
const Anukul = () => {
  const [state, setState] = useState<AnukulState>({
    curSidebar: 0,
    curMidbar: 0,
    midbarList: anukul[0].md,
    contentID: anukul[0].md[0].id,
    contentURL: anukul[0].md[0].file
  });

  const setMidBar = (items: AnukulMdData[], index: number) => {
    setState({
      curSidebar: index,
      curMidbar: 0,
      midbarList: items,
  ...
  return (
    <div className="anukul-app font-avenir flex h-full">
